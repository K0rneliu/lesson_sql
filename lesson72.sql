...
-- добавляем даннв=ые в таб. orders
INSERT INTO orders
  (user_id)
VALUES
  (2),
...
SELECT
  u.id AS iser_id, u.name, o.id AS order_id
FROM users AS u
...
CREATE TABLE flights (
  id SERIAL PRIMARY KEY,
  `from` VARCHAR(255) COMMENT 'взлет',
  `to` VARCHAR(255) COMMENT'посадка'
  ) COMMENT = 'АвиаРейсы';
  
  DROP TABLE IF EXISTS cities;
CREATE TABLE cities (
  id SERIAL PRIMARY KEY,
  label VARCHAR(255) COMMENT 'название города En',
  name VARCHAR(255) COMMENT 'название города Ru'
    ) COMMENT = 'Города';
